...
const snake = {
    ...
    getNextPosition() {
        ...
        // проверка врезания змейки.   
        for(i in this.parts) 
        {       
            //проверка хода в хвост
            if(position.top == this.parts[0].top && position.left == this.parts[0].left)
            {
                break;
            }
            else
...
const food = {
    ...
    removeItem(foundPosition) {
        ...
        let elem = document.getElementById('score-value').textContent;
        var numEl = +elem;
        document.getElementById('score-value').innerText = ++numEl;
    },
...
function init() {
    ...
    setInterval(function(){game.move} , 60)
